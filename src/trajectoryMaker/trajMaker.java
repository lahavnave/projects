...
public class trajMaker {
    public static void main(String[] args) {
        File file = new File("C:\\Users\\RoboMentor\\AndroidStudioProjects\\CenterStage2024\\TeamCode\\trajectories\\RedFar");

        for (File trajectoryFile : file.listFiles()) {
            if (trajectoryFile.getName().charAt(0) != '_') {
                String[][] ValueArray = convertFileToValueArray(trajectoryFile);

                Position[] poses = new Position[ValueArray.length -1];

                for (int i = 0; i < ValueArray.length - 1; i++){
                    poses[i] = new Position(
                            Double.parseDouble(ValueArray[i][0]),
                            Double.parseDouble(ValueArray[i][1]),
                            Double.parseDouble(ValueArray[i][2]),
                            Double.parseDouble(ValueArray[i][3]),
                            i,
                            ValueArray[i][4],
                            "",
                            "");
...
//                            trajectoryFile.getName().split(".yaml")[0].split("_to_")[1]);
                ...
                poses[ValueArray.length - 2].index = -1;
    ...
    static class Position {
        ...
        public Position(double x, double y, double heading, double tangent, int index, String moveType, String from, String to){
            ...
            this.from = "from"; //todo need to fix this
            this.to = "to";
            this.index = index;
        }
